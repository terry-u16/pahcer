...
コンパイルステップを実行するかどうか OS によって選択できます。 `os_regex` が指定されていない場合は全ての OS で、指定されている場合はその正規表現にマッチしたときだけ実行されます。

例:
- os_regex="windows"
- os_regex="(windows|macos)"

使用している OS がどの文字列にマッチするかは https://doc.rust-lang.org/std/env/consts/constant.OS.html を参照してください。
...
テストステップを実行するかどうか OS によって選択できます。 `os_regex` が指定されていない場合は全ての OS で、指定されている場合はその正規表現にマッチしたときだけ実行されます。
